...
export const meetingsAPI = {
  ...
  generateSummary: (meetingId) => api.post(`/meetings/${meetingId}/summary`),

  regenerateSummary: (meetingId, customPrompt) =>
    api.put(`/meetings/${meetingId}/summary/regenerate`, { customPrompt }),

  deleteSummary: (meetingId) => api.delete(`/meetings/${meetingId}/summary`),

  updateActionItem: (meetingId, itemIndex, updates) =>
    api.patch(
      `/meetings/${meetingId}/summary/action-items/${itemIndex}`,
      updates,
    ),
};
...
export const actionItemsAPI = {
  ...
  addActionItem: (meetingId, data) =>
    api.post(`/action-items/${meetingId}`, data),

  // Get meetings hosted by current user
  getHostMeetings: () => api.get("/action-items/host-meetings"),
};
...
export const analyticsAPI = {
  ...
  exportAnalyticsJSON: async (timeRange = 30, userId = null) => {
    try {
      const [overview, trends, speaking, actions, sentiment] =
        await Promise.all([
