...
// "none" disables language-specific stemming so Devanagari, Hindi, Marathi
// and other non-Latin scripts are indexed correctly without MongoDB dropping tokens.
...
transcriptSchema.methods.updateFromFile = async function (transcriptData) {
  ...
  this.stats = {
    ...
    speakingDuration: this.segments.reduce(
      (sum, s) => sum + (s.end - s.start),
      0,
    ),
    averageConfidence:
      this.segments.reduce((sum, s) => sum + (s.confidence || 0), 0) /
...
transcriptSchema.statics.findByMeeting = function (meetingId) {
  return this.find({ meetingId, processingStatus: "completed" }).sort({
    createdAt: 1,
  });
};

transcriptSchema.statics.searchTranscripts = function (
  query,
  meetingId = null,
) {
  const searchQuery = {
