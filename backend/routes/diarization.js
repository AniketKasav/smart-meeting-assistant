...
router.post('/:meetingId/run', async (req, res) => {
  try {
    ...
    // ✅ FIX: Construct audioPath if missing
    let audioPath = transcript.audioPath;
    
    if (!audioPath) {
      console.log('[diarization] audioPath missing, attempting to construct...');
      
      // Try to construct the path based on meetingId
      const possiblePaths = [
        path.join(__dirname, '../uploads', transcript.meetingId, 'audio.wav'),
        path.join(__dirname, '../uploads', transcript.meetingId, 'recording.wav'),
      ...
      for (const possiblePath of possiblePaths) {
        if (fs.existsSync(possiblePath)) {
          ...
          // Update the transcript with the found audioPath
      ...
      // If still not found, check uploads directory
      if (!audioPath) {
        const uploadsDir = path.join(__dirname, '../uploads', transcript.meetingId);
    ...
    if (!audioPath) {
      console.error('[diarization] audioPath still missing after search');
      return res.status(400).json({ 
        error: 'Audio file not found. Please ensure the meeting was recorded with audio.',
        meetingId: transcript.meetingId
      });
    }

    if (!fs.existsSync(audioPath)) {
      console.error('[diarization] Audio file does not exist:', audioPath);
